...
//access_token^timestamp^clientid
        build.append(access_token).append("^").append(timeStamp).append("^").append(client_id);
        StringBuilder url = new StringBuilder(ARTICLE_SAVE_URL);
    ...
    public static void getRelateArticle(final Context context, String id,int page,final OnNetResponseListener listener){
        long timeStamp= System.currentTimeMillis();
        String access_token =LoginUserManager.getInstance().access_token;
        String client_id =LoginUserManager.getInstance().client_id;
        String sign= generateMd5Str(access_token,timeStamp,APP_KEY,client_id);
...
//access_token^timestamp^clientid
    ...
    public static void getFavoriteList(final Context context, int page, final OnNetResponseListener listener){
        long timeStamp= System.currentTimeMillis();
...
//access_token^timestamp^clientid
        ...
        StringBuilder url = new StringBuilder(FAVORITE_URL);
        url.append("&page=").append(page);
        OkGo.get(url.toString()).headers("Authorization",sign+build.toString()).execute(new StringNetCallback(context) {
            @Override
            public void onSuccess(String s, Call call, Response response) {
    ...
    public static void getMessageList(final Context context, final OnNetResponseListener listener){
        long timeStamp= System.currentTimeMillis();
...
//access_token^timestamp^clientid
        ...
        OkGo.get(MESSAGE_URL).headers("Authorization",sign+build.toString()).execute(new StringNetCallback(context) {
            @Override
            public void onSuccess(String s, Call call, Response response) {
                paraJson(context,s,listener);
            }
...
//access_token^timestamp^clientid
    ...
    public static void editEmail(final Context context, String old_email, String old_ecode,
                                 String email, String ecode,
                                 final OnNetResponseListener listener){
        long timeStamp= System.currentTimeMillis();
...
//access_token^timestamp^clientid
        ...
        params.put("old_ecode", old_ecode);
        params.put("email", email);
...
//access_token^timestamp^clientid
    ...
    public static void getPersonalInfo(final Context context, final OnNetResponseListener listener){
        long timeStamp= System.currentTimeMillis();
